...
class MatrizGenerator {
    ...
    func showHint() {
        ...
        case .horizontal:
            ...
            generatedGrid[line][column + randomLetter] = String(Array(word.name)[randomLetter])
            
        case .vertical:
            let randomLetter = Int.random(in: 0..<wordSize)
            generatedGrid[line + randomLetter][column] = String(Array(word.name)[randomLetter])
        }
    }
        ...
        for word in words {
            ...
            case .horizontal:
                for _ in colunm..<colunm + wordSize {
                    generatedGrid[line][colunm + letter] = generatedGrid[line][colunm + letter]
                    letter += 1
                }
                
            case .vertical:
                for _ in line..<line + wordSize {
                    generatedGrid[line + letter][colunm] = generatedGrid[line + letter][colunm]
                    letter += 1
                }
            }
        }
    ...
    func exitGame() {
        var quit: String
        repeat {
            print("ARE YOU SURE YOU WANT TO LEAVE?", terminator: "")
